/*
 * AppConstants
 * These are the variables that determine what our central data store (reducer.js)
 * changes in our state. When you add a new action, you have to add a new constant here
 *
 * Follow this format:
 * export const YOUR_ACTION_CONSTANT = 'YOUR_ACTION_CONSTANT';
 */

export const REQUEST_FEED = 'REQUEST_FEED';
export const RECEIVE_FEED = 'RECEIVE_FEED';
export const REQUEST_ADDITIONAL_FEED = 'REQUEST_ADDITIONAL_FEED';
export const RECEIVE_ADDITIONAL_FEED = 'RECEIVE_ADDITIONAL_FEED';


export const RECEIVE_ENTITIES = 'RECEIVE_ENTITIES';
export const REQUEST_ENTITIES = 'REQUEST_ENTITIES';
export const RECEIVE_ENTITY = 'RECEIVE_ENTITY';

export const REQUEST_AUTHOR = 'REQUEST_AUTHOR';
export const RECEIVE_AUTHOR = 'RECEIVE_AUTHOR';

export const REQUEST_PUBLISHER = 'REQUEST_PUBLISHER';
export const RECEIVE_PUBLISHER = 'RECEIVE_PUBLISHER';

export const REQUEST_ADDITIONAL_ENTITY_ARTICLES = 'REQUEST_ADDITIONAL_ENTITY_ARTICLES';
export const RECEIVE_ADDITIONAL_ENTITY_ARTICLES = 'RECEIVE_ADDITIONAL_ENTITY_ARTICLES';
export const REQUEST_ENTITY_ARTICLES = 'REQUEST_ENTITY_ARTICLES';
export const RECEIVE_ENTITY_ARTICLES = 'RECEIVE_ENTITY_ARTICLES';

export const REQUEST_ADDITIONAL_PUBLISHER_ARTICLES = 'REQUEST_ADDITIONAL_PUBLISHER_ARTICLES';
export const RECEIVE_ADDITIONAL_PUBLISHER_ARTICLES = 'RECEIVE_ADDITIONAL_PUBLISHER_ARTICLES';
export const RECEIVE_PUBLISHER_ARTICLES = 'RECEIVE_PUBLISHER_ARTICLES';

export const REQUEST_ARTICLES = 'REQUEST_ARTICLES';
export const RECEIVE_ARTICLES = 'RECEIVE_ARTICLES';
